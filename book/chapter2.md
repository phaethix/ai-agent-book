...
需要补充的是，把业务背景整理成上下文、而不是硬编码进代码逻辑（比如用大量 if-else 规则处理每种业务场景），与 Sutton 的“苦涩的教训”之间存在一层值得玩味的张力。一方面，用上下文承载知识比把规则写死更通用，也更能随模型能力的增长而持续受益；另一方面，无论写进上下文还是写进代码，精心编排的业务知识本身仍是“人的先验”，而“苦涩的教训”真正推崇的是让系统靠算力自己学到这些知识[^ch2-1]。本书的立场是：上下文工程是当下用好现有模型的务实手段，而让 Agent 自己发现知识结构（第八章的自我进化）才是更长期的方向。本章末尾会回到这条主线展开。

[^ch2-1]: Rich Sutton, “The Bitter Lesson” , 2019. http://www.incompleteideas.net/IncIdeas/BitterLesson.html

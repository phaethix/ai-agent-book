...
- **第一章**建立公式的三层理解——实现层、直觉层、学术层，并给出从工作流到自主 Agent 的编排光谱。
- **第二、三章——“Agent 看到什么”。** 上下文工程决定单次会话中模型眼前的世界；用户记忆与知识库再把这个世界从单次会话扩展为跨会话的长期积累。
- **第四、五章——“Agent 能做什么”。** 工具定义能力边界；Coding Agent 之所以被单独展开，是因为代码是通用性最强的工具——能创造新工具的工具。
- **第六、七章——“如何度量和放大智能”。** 评估把 Agent 的表现变成可比较的信号，后训练再把这些信号变成模型自身的能力。
- **第八、九章——拓展边界。** 自我进化让 Agent 在不改权重的前提下积累经验；多模态把感知和行动从文本扩展到视觉、语音与物理世界。
- **第十章——更大尺度上的复用。** 多 Agent 协作不是公式之外的新东西：上下文是否共享，是第二章“隔离优于压缩”在系统架构层的表达；“Agent 互为工具”直接来自第四章的协作工具设计；判断多 Agent 优劣的“新信息”判据，则呼应第六章评估的核心思想。

理解了单 Agent 的三要素，多 Agent 系统的每一个设计决策都能在其中找到对应。这个公式不是一份记着“已经做成哪些事”的清单，而是一副透镜——它向内解释全书讲过的一切，向外照见那些还没做成的地方。

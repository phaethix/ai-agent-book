...
- **程序记忆**（Procedural Memory）：关于行为模式和流程的记忆。人类例子：骑自行车的能力。Agent 对应：从用户反复订机票的模式中学到的通用流程——“先搜索直飞航班→确认座位偏好→使用常旅客号码→订餐”。这里要加一点限定：认知科学中的程序记忆是**内隐的、非陈述性的**——会骑车却难以用语言讲清每一步该如何发力；而 Agent 的对应物恰恰是被显式写成文本的流程知识，严格说更接近“陈述化的程序性知识”。借用这个类比是为了对应“流程”这一内容类型，而非声称二者的存储机制相同。
...
以上司法案例说明了 Agent 与领域知识结合的具体路径。最后，我们跳出本章具体技术，借用 AI 领域一对经典概念做一次更高层次的反思。Rich Sutton 在《苦涩的教训》（The Bitter Lesson，2019）中回顾 AI 七十年的历史，指出依靠算力与数据扩展的**通用方法**（搜索与学习——早期 AI 文献称之为“弱方法”，因为它们不依赖领域知识）总是最终胜过依靠人工编码领域知识的**强方法**——后者短期见效快，长期却成为扩展的天花板。本章构建的知识系统恰好站在这对张力的交汇处：RAG 检索与 LLM 自动归纳属于通用方法，而实验 3-13 的“自下而上因子发现 + 人工审核”，则是有意识地在通用方法之上保留一层人类知识的把关。

**通用方法的边界**。通用方法虽然可扩展性强大，但在**高可靠性关键系统**（如自动驾驶安全规则、碰撞避免算法）中仍需形式化的、可证明的保障作为“安全网”；在**数据稀缺领域**，人类专家构建的知识结构可作为“先验”引导高效学习——这正是实验 3-13 采用“自下而上因子发现 + 人工审核”而非完全自动化的原因。因此，构建真正鲁棒的 Agent 系统，需要的不是在通用方法和强方法之间二选一，而是将两者巧妙结合：用通用方法的可扩展性来突破人类知识的天花板，用强方法的可靠性来构筑系统的安全底线，用人类知识的结构性来提升小数据场景下的学习效率。这种“混合智能”的哲学，将是下一代 AI 系统设计的核心。

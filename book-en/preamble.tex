...
% Sans stays the class default (TeX Gyre Heros, Helvetica clone) — already
% equivalent to the papers' helvet. unicode-math loads after the class's
% amsmath, so math switches from Computer Modern to Pagella Math coherently.
\usepackage{unicode-math}
% Loaded by filename (like the class's own TeXGyreTermesX setup): macOS
% fontconfig doesn't index texmf-dist, so family-name lookup would fail.
